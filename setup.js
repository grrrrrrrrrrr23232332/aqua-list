// Setup script to create config.json and .env files
const fs = require("fs")
...
async function setup() {
  ...
  const discordClientSecret = await askQuestion("Enter your Discord Client Secret: ")
  ...
  const discordLogChannelId = await askQuestion("Enter your Discord Log Channel ID: ")

  // NextAuth
  const nextAuthSecret = await askQuestion("Enter your NextAuth Secret (or press Enter to generate one): ")
  const generatedSecret =
    nextAuthSecret || Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15)

  // Create .env file
  const envContent = envTemplate
    .replace("MONGODB_URI=", `MONGODB_URI=${mongodbUri}`)
    .replace("DISCORD_CLIENT_ID=", `DISCORD_CLIENT_ID=${discordClientId}`)
    .replace("DISCORD_CLIENT_SECRET=", `DISCORD_CLIENT_SECRET=${discordClientSecret}`)
    .replace("DISCORD_BOT_TOKEN=", `DISCORD_BOT_TOKEN=${discordBotToken}`)
    .replace("DISCORD_SERVER_ID=", `DISCORD_SERVER_ID=${discordServerId}`)
    .replace("DISCORD_LOG_CHANNEL_ID=", `DISCORD_LOG_CHANNEL_ID=${discordLogChannelId}`)
